...
Are difference between 2018 PA vs 2018 NJ significant = no p = 0.993
...
summary(ticksex_glm)
```
Powassan significant between male and female = no significance p= 0.379
...
```
Lyme data for tick sex male vs female significance = no(?) p = 0.817
...
summary(POWNJ_glm)
```
POW Significant difference between 2015-2018 NJ = no significance p =0.261
...
summary(NJL_glm)
```
Significance b/w NJ 2015-2018 for Lyme = no significance p = 0.261
...
summary(POWPA_glm)
```
POW significance between PA 2017-2018 = no significance p = 0.359
...
summary(PAL_glm)
```
Significanc for Lyme PA 2017 vs 2018 = no significance p = 0.787
...
summary(PANJPOW_glm)
```
Significace between POW PA vs NJ overall = no significance p = 0.377
...
```
PA vs NJ Lyme = no significane  p = 0.807
...
```
No engorgment significance for POW in adults p =0.347
...
```
no significance for engorgment in adults for Lyme p= 0.730
...
```
Nymp no significance for engorment and POW p=0.359
...
```
nymph lyme engorgement = no signicance p=0.294
...
summary(Lstage_glm)
```
lyme male vs female vs nymph no significance p = 0.532
...
summary(Pstage_glm)
```
Pow Male vs female vs nymph no significance p = 0.273
